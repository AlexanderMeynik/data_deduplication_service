...
namespace db_services {
    ...
    conPtr dbManager<segment_size>::connectToDb() {
        ...
        if (verbose >= 1) {
            std::cout << "Failed to listen with current connection, attempt to reconnect via cString\n";
        }
    ...
    void dbManager<segment_size>::create() {
        ...
        //auto name = vformat("%s_%d", tString.dbname.c_str(), segment_size);

        pqxx::nontransaction no_trans_exec(*temp_connection);

        try {
            no_trans_exec.exec(vformat("CREATE DATABASE %s;", cString_.dbname.c_str()));
        } catch (pqxx::sql_error &e) {
            if (e.sqlstate() == "42P04") {
                if (verbose >= 2) {
                    std::cout << vformat("Database: %s exists, proceeding further\n", cString_.dbname.c_str());
                }
                temp_connection->disconnect();
                return;
            }
            if (verbose >= 1) {
                std::cout << vformat("Database error: %s, error code: %sSQL Query was: %s\n",
                                     cString_.dbname.c_str());
            }
            return;
        }

        no_trans_exec.exec(vformat("GRANT ALL ON DATABASE %s TO %s;", cString_.dbname.c_str(), cString_.user.c_str()));
        no_trans_exec.commit();
        temp_connection->disconnect();

        conn_ = connect_if_possible<verbose>(cString_);
        std::cout<<conn_->dbname()<<'\n';
    }
    ...
    void dbManager<segment_size>::fill_schemas() {
        try {
            ...
            if (verbose >= 2) {
                std::cout <<"Create segments table " <<r1.query()<< '\n';
            }
            ...
            if (verbose >= 2) {
                std::cout <<"Create other tables " <<r1.query()<< '\n';
            }
            ...
            if (verbose >= 2) {//todo extract
                std::cout <<"Create  functions and procedures " <<r1.query()<< '\n';
            }
            ...
            query="CREATE OR REPLACE FUNCTION get_file_data(fileName text)\n"
                  ...
                  "\n"
                  "END $$;";
            q1= vformat(query.c_str(),segment_size);
            r1 = txn.exec(q1);


            if (verbose >= 2) {
                std::cout <<"Create function get_file_contents " <<r1.query()<< '\n';
            }
            ...
            if (verbose >= 1) {
                std::cout << "Error inserting bulk data: " << e.what() << '\n';
            }
        }
    }
    ...
    void dbManager<segment_size>::insert_bulk_segments(const segvec<segment_size> &segments, std::string &filename) {
        try {
            pqxx::work txn(*conn_);
            std::string table_name = "temp_file_" + filename;
            std::string q1 = vformat("CREATE TABLE %s (pos bigint, data char(%d));", table_name.c_str(), segment_size);
            //"CREATE TABLE " + table_name + " (pos bigint, data char(" + std::to_string(segment_size) + "));";
    ...
    segvec<segment_size> dbManager<segment_size>::get_file_segmented(std::string &filename) {
        ...
        try {
            ...
            if (verbose >= 1) {
                std::cout << "get_file_segments" << e.what() << std::endl;
            }
        }
        return vector;
    }
