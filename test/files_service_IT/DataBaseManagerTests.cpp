...
{
   auto a= wrap_non_trans_method<64, &dbManager<64>::checkDatabaseExistence>(DbMangement_tests::manager_);
...
TEST_F(DbMangement_tests, test_schema_creation)
{
    ...
    auto pqres= wrap_trans_method<64, &dbManager<64>::checkSchemas>(DbMangement_tests::manager_);
    ASSERT_TRUE(pqres.has_value());
    ASSERT_EQ(pqres.value().size(), 0);

    manager_.fill_schemas();

    pqres = wrap_trans_method<64, &dbManager<64>::checkSchemas>(DbMangement_tests::manager_);
    ASSERT_TRUE(pqres.has_value());
...
{
    auto new_db_name=DbMangement_tests::dbName+"_";
    ...
    a= wrap_non_trans_method<64, &dbManager<64>::checkDatabaseExistence>(DbMangement_tests::manager_);
    ASSERT_TRUE(a.has_value());
